...
export default function StoreDetailPage() {
  ...
  const {
    ...
    enabled: !!id, // 아이디가 있을때만 사용이 가능하게! 없으면 안됨!
    refetchOnWindowFocus: false
  });
  const handleDelete = async () => {
    const confirm = window.confirm("해당 가게를 삭제 하시겠습니까?");
    if (confirm) {
      try {
  ...
  return (
    <>
          ...
          {status === "authenticated" && store && (
            <div className="md:flex justify-between items-center gap-4 px-4 py-3">
              ...
              <button
                ...
                className="underline hover:text-gray-400 text-sm px-4 py-3"
              >
                삭제
              </button>
            </div>
      ...
      {isSuccess && (
        <>
          <div className="overflow-hidden w-full mb-20 max-x-5xl mx-auto max-h-[600px]">
